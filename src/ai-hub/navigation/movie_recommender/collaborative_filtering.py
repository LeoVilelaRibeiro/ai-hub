...
st.markdown(
    """
  This page recommends movies based on **Collaborative Filtering**, using the
  **MovieLens dataset**.

  Given a movie you've watched, it suggests other movies that were rated by users who
  also rated the selected movie.
...
common_counts = (
    pivot_table.notnull()
    .astype(int)
    .T.dot(pivot_table.notnull().astype(int))[selected_movie_id]
)

recommendations = pd.DataFrame(
    {"Similarity": similarities, "Common Ratings": common_counts}
).reset_index()
...
if recommendations.empty:
    ...
    st.dataframe(
        recommendations[
            ["Recommended Movie", "Similarity", "Common Ratings", "genres"]
        ],
