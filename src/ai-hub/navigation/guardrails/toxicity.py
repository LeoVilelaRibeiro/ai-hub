...
st.markdown(
    """
  This page uses the **`unitary/toxic-bert`** model via the **Hugging Face Inference
  API**
...
if run:
    with st.spinner("Analyzing..."):
        ...
        if isinstance(result, dict) and result.get("error"):
            ...
            toxic_entry = next(
                (item for item in result[0] if item["label"] == "toxic"), None
            )
            if toxic_entry:
                if toxic_entry["score"] > 0.5:
                    st.warning(
                        "☣️ This sentence is likely toxic. Choose your words wisely."
                    )
                else:
                    st.success(
                        "🌼 This sentence seems civil and respectful. Nicely done!"
                    )
            else:
